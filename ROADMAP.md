...
| [R8.3](#r83--android-1516-edge-to-edge-audit-mandatory-at-targetsdk--36) | Android 15/16 edge-to-edge audit | Done: MainActivity calls `enableEdgeToEdge()` and now uses `safeDrawingPadding()` on the NavHost root (replaces `systemBarsPadding` — also covers cutouts + IME); `Window.isNavigationBarContrastEnforced = false` (API 29+ guarded) drops the 3-button-nav scrim. No display-cutout regressions found across screen surfaces. |
...
- The NavHost root modifier now uses `safeDrawingPadding()` (was `systemBarsPadding()`) so every nav destination (projects, settings, editor) keeps its content out of system bars **and** display cutouts **and** the IME — no per-screen `Scaffold` rewrite required at this stage. ✓
...
- Display cutout: focused grep across screen surfaces showed no manual horizontal layout that would clip under a left/right cutout, and `safeDrawingPadding` covers the cutout safe zone explicitly. Re-audit once R8.8 adaptive resizability work surfaces a Pixel Fold inner-display layout that uses the full horizontal width.
