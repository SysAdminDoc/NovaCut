...
- [~] **ExportSheet third confidence row "AI use"** — Data layer shipped: `AiUsageLedger.summarizeForChips(entries)` returns a stable-ordered `List<Chip>` (severity descending then effect-kind alphabetical) with bucketed entry count / clip count / total range / distinct model names. `Chip.describe()` and `Chip.effectKindLabel` provide formatted body text without string-resource coupling. 8 new tests in `AiUsageLedgerChipsTest`. Compose row in `ExportSheet.kt` is a follow-up commit; the data contract is now stable to consume.
...
- [~] **`AiToolRequirements` registry shipped** — Pure-Kotlin per-tool requirement table for 16 AI tools (auto captions, BG removal, AI background, stabilize, style, upscale, frame interp, object remove, tap segment, noise reduction, stem separation, generative video, lip sync, voice clone, TTS, caption translate). `requirementFor(toolId)` returns `ToolRequirement(modelDisplayName, estimatedBytes, license, sourceUrl, runtimeLocation, availability, requiresOptInConsent)`. 8 tests lock invariants: every tool has an entry, cloud tools always require consent and are never READY, on-device tools have non-zero estimates, at least one tool is READY (DeepFilterNet) so the sheet has a positive path. Compose `AiModelRequirementSheet` composable + `AiToolsPanel` adoption is a follow-up commit consuming this registry.
...
- [~] **Chip-bucket helpers shipped** — `SilenceDetectionEngine.ProposalCategory { SILENCE / SINGLE_WORD_FILLER / MULTI_WORD_FILLER / OTHER }` + `categorize(proposal)` + `filterByCategory(proposals, enabled)` (empty enabled returns empty list, full enabled returns same-instance identity filter) + `groupByCategory(proposals)` (LinkedHashMap, preserves input order within bucket). 8 new tests in `SilenceDetectionCategoryTest`. Compose panel consolidation — chip row in `CutAssistantReviewPanel` + deletion of `FillerRemovalPanel` once `AiToolsDelegate` routes are migrated — is a follow-up UI commit.
