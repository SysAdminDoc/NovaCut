...
| [R6.10](#r610--media3-110-modular-ui-adoption) | Adopt Media3 1.10 modular pieces where parity holds | Done for Lottie: official Media3 renderer handles eligible exports, with fallback retained where Media3 would change output semantics. Done for Player Compose evaluation: keep NovaCut's custom preview for timeline-relative seeking, gap/still-image states, transform gestures, scopes, and Mocha chrome. Done for inspector-frame audit: current cached SDR thumbnails stay on `MediaMetadataRetriever`; HDR/effect/custom-decoder frame work must use `media3-inspector-frame`. Remaining R6.10 work is low-priority progress-slider watch. |
