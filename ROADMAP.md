...
| [R8.8](#r88--android-16-adaptive-resizability-tablets--foldables) | Adaptive resizability audit for sw≥600dp | In progress: manifest explicitly opts into resizable activity behavior and expanded config changes; [AdaptiveEditorLayoutPolicy](app/src/main/java/com/novacut/editor/ui/editor/AdaptiveEditorLayoutPolicy.kt) classifies compact/medium/expanded/tabletop/desktop pane modes; 840dp+ tablets now get the existing desktop sidebar/roomier timeline sizing; MediaPicker accepts external drag/drop media URIs. Remaining: actual WindowManager folding-feature posture feed and ActivityEmbedding export/editor split. |
| [R8.9](#r89--ai-disclosure-ux-pairs-with-c2pa-and-generative-policy) | AI-generated content disclosure UX | In progress: [AiUsageLedger](app/src/main/java/com/novacut/editor/engine/AiUsageLedger.kt) + autosave persistence are live; ExportSheet now defaults "Disclose AI use" on for any non-empty ledger, writes a matching `.ai-use.json` sidecar, and exposes a clear/review flow; DirectPublishEngine includes AI disclosure text for YouTube/TikTok-style targets; PrivacyDashboard has an AI ledger row. Remaining: hook every generative engine into `recordAiUsage`, embed C2PA / MP4 `udta` metadata, and add real API-upload checkbox handling once those platform upload paths ship. |
...
| [R8.2](#r82--c2pa-content-credentials-for-mp4-export) | C2PA Content Credentials on MP4 export | Engine scaffold done: [C2paExportEngine](app/src/main/java/com/novacut/editor/engine/C2paExportEngine.kt) builds a structured `C2paManifest` (training-mining opt-out, thumbnail claim, `c2pa.created` action, AI-actions assertion derived from `AiUsageLedger`) with `SigningMode` ANDROID_KEYSTORE / STRONGBOX / USER_PEM / WEB_SERVICE. AI ledger entries are mapped to canonical C2PA actions (`c2pa.created` / `c2pa.edited` / `c2pa.opinion`) with v2 `digitalSourceType` (`trainedAlgorithmicMedia` / `compositeWithTrainedAlgorithmicMedia` / `algorithmicMedia`). 13 new tests. `signAndEmbed` returns `Unavailable` until the c2pa-android AAR is wired; remaining is the AAR addition + 16 KB verification + license addendum. |
| [R8.5](#r85--thermal-aware-export-scheduling) | Thermal-aware export scheduling | Policy engine done: [ThermalHeadroomPolicy](app/src/main/java/com/novacut/editor/engine/ThermalHeadroomPolicy.kt) translates `(ThermalStatus, headroom)` → `ThrottleDecision` (FULL_SPEED / THROTTLE_LIGHT / THROTTLE_HEAVY / PAUSE / CANCEL) with maxParallelPasses, proxy-fallback, and notification debouncing. Includes `shouldOfferOvernightSchedule` for long renders. 12 new tests. Remaining: wire `ExportService` to register `addThermalStatusListener` + poll `getThermalHeadroom(30)` once per second and act on the `ThrottleDecision`. |
...
- Remaining: feed real `WindowInfoTracker` / `FoldingFeature` posture into `AdaptiveEditorLayoutPolicy` so tabletop mode can place preview above timeline across the hinge.
- Remaining: add `androidx.window.embedding.ActivityEmbeddingController` rules for a true side-by-side ExportSheet + EditorScreen split on Pixel Tablet / desktop windowing.
...
- Remaining: hook each generative / substantial AI engine into `EditorViewModel.recordAiUsage`.
