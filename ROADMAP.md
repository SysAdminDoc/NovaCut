...
- [ ] P2 - Premium Mobile Editor UX Pass
  Why: Dense tool trays, selected states, empty/error/loading states, touch
  targets, contrast, and accessible focus should feel intentionally designed on
  phone layouts.
  Where: app/src/main/java/com/novacut/editor/ui
- [ ] P2 - Project Inspector
  Why: A project details panel should show media count, storage used, missing
  files, export presets, last saved, schema version, and recovery snapshots to
  help users and diagnostics.
  Where: app/src/main/java/com/novacut/editor/ui/projects, app/src/main/java/com/novacut/editor/engine
...
- [ ] P1 — Export failure incident bundle
  Why: Export failures, timeouts, empty outputs, and device encoder quirks need one redacted support artifact instead of scattered history rows, logs, and generic diagnostic ZIP data.
  Evidence: RESEARCH.md; app/src/main/java/com/novacut/editor/engine/VideoEngine.kt; app/src/main/java/com/novacut/editor/engine/ExportHistoryStore.kt; app/src/main/java/com/novacut/editor/engine/DiagnosticExportEngine.kt; Media3 Transformer docs; CapCut community export-failure reports.
  Touches: app/src/main/java/com/novacut/editor/engine/VideoEngine.kt, app/src/main/java/com/novacut/editor/ui/editor/ExportDelegate.kt, app/src/main/java/com/novacut/editor/engine/ExportHistoryStore.kt, app/src/main/java/com/novacut/editor/engine/DiagnosticExportEngine.kt, app/src/main/java/com/novacut/editor/ui/export/ExportSheet.kt, app/src/test/java/com/novacut/editor/engine/
  Acceptance: failed/cancelled/timeout exports create a bounded redacted incident JSON and optional share bundle with app version, export config, encoder path, media-health summary, progress samples, failed phase, error class/message, and no raw media paths, captions, transcripts, or project JSON; tests verify bundle shape and redaction.
  Complexity: M

- [ ] P1 — Audio conformance preflight and resampling gate
  Why: Audio sample-rate/channel mismatches are a recurring editor sync/volume failure mode, and NovaCut's export preflight currently checks media health/relink only while the Oboe resampler remains unwired.
  Evidence: RESEARCH.md; app/src/main/java/com/novacut/editor/ui/editor/ExportMediaPreflight.kt; app/src/main/java/com/novacut/editor/engine/OboeResamplerEngine.kt; OpenShot sample-rate sync reports; CapCut audio/video desync reports.
  Touches: app/src/main/java/com/novacut/editor/ui/editor/ExportMediaPreflight.kt, app/src/main/java/com/novacut/editor/engine/AudioEngine.kt, app/src/main/java/com/novacut/editor/engine/AudioEffectsEngine.kt, app/src/main/java/com/novacut/editor/engine/VolumeAudioProcessor.kt, app/src/main/java/com/novacut/editor/engine/OboeResamplerEngine.kt, media import/probe code, app/src/test/java/com/novacut/editor/engine/
  Acceptance: import/export preflight reports sample rate, channel count, duration drift, and variable-frame-rate/audio risk; export either normalizes all mixed audio to the selected output sample rate/channel layout or blocks with a repair message; tests cover 44.1kHz to 48kHz, mono/stereo, missing metadata, and drift warnings.
  Complexity: L

### P2

- [ ] P2 — Bounded native media and model processing policy
  Why: FFmpeg and ONNX paths parse attacker-controlled media/model bytes; dependency checks are necessary but do not bound runtime size, duration, format, timeout, or checksum behavior per operation.
  Evidence: RESEARCH.md; app/src/main/java/com/novacut/editor/engine/FFmpegEngine.kt; app/src/main/java/com/novacut/editor/engine/ModelDownloadManager.kt; app/src/main/java/com/novacut/editor/engine/InpaintingEngine.kt; FFmpeg security advisories; ONNX Runtime 1.26 release notes.
  Touches: app/src/main/java/com/novacut/editor/engine/FFmpegEngine.kt, app/src/main/java/com/novacut/editor/engine/ModelDownloadManager.kt, app/src/main/java/com/novacut/editor/engine/InpaintingEngine.kt, AI/native engine call sites, diagnostics, app/src/test/java/com/novacut/editor/engine/
  Acceptance: every FFmpeg/ONNX entry point runs through a shared policy with max bytes, max duration, supported MIME/extension checks, required checksums where applicable, per-operation timeouts, cancellation cleanup, and sanitized user/diagnostic failure reasons; corrupt, oversized, unsupported, and checksum-mismatch fixtures fail closed in tests.
  Complexity: M

- [ ] P2 — Install `.ncstyle` caption and text style packs
  Why: Templates and style packs are table-stakes in commercial mobile editors, and NovaCut already recognizes `.ncstyle` files but blocks installation.
  Evidence: RESEARCH.md; docs/templates.md; app/src/main/java/com/novacut/editor/engine/PluginRegistry.kt; app/src/main/java/com/novacut/editor/engine/IncomingDocumentImportRouter.kt; KineMaster/VN/CapCut template and asset surfaces.
  Touches: app/src/main/java/com/novacut/editor/engine/IncomingDocumentImportRouter.kt, app/src/main/java/com/novacut/editor/engine/PluginRegistry.kt, app/src/main/java/com/novacut/editor/ui/editor/CaptionStyleGallery.kt, text/caption editor panels, style-pack storage, license/attribution surfaces, app/src/test/java/com/novacut/editor/engine/
  Acceptance: importing `.ncstyle` validates schema version, NovaCut compatibility, resource sizes, embedded fonts/assets, license/attribution metadata, and duplicate IDs; valid packs install into a local registry and appear in caption/text style pickers; invalid packs show a structured rejection report and mutate no project state.
  Complexity: M

- [ ] P2 — Storyboard and shot-plan workspace for auto-edit
  Why: Creator planning/storyboards are a competitive differentiator in Meta Edits, and NovaCut has a scratchpad plus script-to-segment parser but no structured shot list that can survive autosave and seed an edit.
  Evidence: RESEARCH.md; app/src/main/java/com/novacut/editor/ui/editor/ScratchpadSheet.kt; app/src/main/java/com/novacut/editor/ui/editor/AutoEditPanel.kt; app/src/main/java/com/novacut/editor/ai/AiFeatures.kt; Meta Edits project-management/storyboard positioning.
  Touches: project/autosave model, archive serialization, app/src/main/java/com/novacut/editor/ui/editor/ScratchpadSheet.kt, app/src/main/java/com/novacut/editor/ui/editor/AutoEditPanel.kt, app/src/main/java/com/novacut/editor/ai/AiFeatures.kt, project list/detail UI, tests
  Acceptance: a project can store ordered storyboard cards with shot text, target duration/aspect, optional linked media placeholder, and status; AutoEdit can consume cards to seed clip order/durations; cards survive autosave/archive/restore and are excluded from diagnostics unless the user explicitly exports project metadata.
  Complexity: M

### P3

- [ ] P3 — dotLottie v2/state-machine compatibility spike
  Why: NovaCut already has Lottie and a parked Rive stub; dotLottie v2/state machines may satisfy interactive title/template needs with less runtime and APK risk than activating Rive first.
  Evidence: RESEARCH.md; docs/templates.md; app/src/main/java/com/novacut/editor/engine/LottieTemplateEngine.kt; app/src/main/java/com/novacut/editor/engine/RiveTemplateEngine.kt; dotLottie v2 specification.
  Touches: app/src/main/java/com/novacut/editor/engine/LottieTemplateEngine.kt, app/src/main/java/com/novacut/editor/engine/RiveTemplateEngine.kt, app/src/main/java/com/novacut/editor/engine/Media3LottieTextureOverlay.kt, gradle/libs.versions.toml, template compatibility tests
  Acceptance: a time-boxed spike imports one `.lottie` state-machine sample, measures APK/runtime impact, verifies text/color/state-machine input support, documents adopt/defer criteria in existing template docs, and leaves Rive inactive unless dotLottie cannot meet the tested template use case.
  Complexity: S

### P1

- [ ] P1 - Post-export playable-output verification gate
  Why: A non-empty export can still be truncated, malformed, missing expected streams, or unreadable, and users should not see share/save success until the produced media is structurally verified.
  Evidence: RESEARCH.md; app/src/main/java/com/novacut/editor/engine/VideoEngine.kt; app/src/main/java/com/novacut/editor/ui/editor/ExportDelegate.kt; app/src/main/java/com/novacut/editor/engine/ExportHistoryStore.kt; Open Video Editor export scan release notes; CapCut export-stuck help; Shotcut 99% export forum; Media3 Transformer docs.
  Touches: app/src/main/java/com/novacut/editor/engine/VideoEngine.kt, app/src/main/java/com/novacut/editor/ui/editor/ExportDelegate.kt, app/src/main/java/com/novacut/editor/engine/ExportHistoryStore.kt, app/src/main/java/com/novacut/editor/engine/DiagnosticExportEngine.kt, app/src/main/java/com/novacut/editor/ui/export/ExportSheet.kt, app/src/test/java/com/novacut/editor/engine/
  Acceptance: every completed Transformer/FFmpeg export enters a verifying phase before COMPLETE; the verifier reopens the output and checks nonzero size, readable metadata, expected video/audio stream presence, duration tolerance, dimensions/rotation, and at least one readable frame/sample when applicable; failed verification marks the export ERROR, removes or quarantines partial public rows, records a redacted report, and enables retry/diagnostic actions without exposing raw paths.
  Complexity: M
...
- [ ] P3 — Auto-suggest optimal export settings from timeline analysis
  ...
  Acceptance: after clips are synced, the multicam panel can build a proposed speaker-angle cut plan, show the speaker-angle map and cut count, allow manual speaker-to-angle overrides and dwell policy selection, preview/reject the plan, and apply accepted cuts as one undoable edit; unavailable states explain missing angles or speaker turns; tests cover planner binding, UI state, no-diarization fallback, and undo/autosave behavior.
  Complexity: L
