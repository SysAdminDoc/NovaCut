...
| [R8.9](#r89--ai-disclosure-ux-pairs-with-c2pa-and-generative-policy) | AI-generated content disclosure UX | Engine + persistence done: [AiUsageLedger](app/src/main/java/com/novacut/editor/engine/AiUsageLedger.kt) records per-`(clipId, effectKind, model, range)` AI usage, classifies into `DISCLOSURE_REQUIRED` / `DISCLOSURE_RECOMMENDED` / `INTERNAL_ONLY`, merges overlaps, and round-trips through `AutoSaveState.aiUsageLedger`. `EditorViewModel.recordAiUsage` / `clearAiUsageLedger` are the call sites. Remaining: hook generative engines into `recordAiUsage`, add "Disclose AI use" export toggle, add Privacy Dashboard row. |
