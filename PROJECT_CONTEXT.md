...
- Advanced R8.8. `MainActivity` explicitly opts into resizable activity behavior; `AdaptiveEditorLayoutPolicy` provides unit-tested compact/medium/expanded/tabletop decisions; expanded 840dp+ windows now get the existing desktop sidebar layout without requiring a mouse/DeX signal; `EditorScreen` applies roomier preview/timeline sizing on medium/expanded windows; `MediaPickerSheet` accepts external drag/drop media URIs through the same local-copy path as picker imports. Remaining R8.8 work is a real WindowManager folding-feature posture feed and ActivityEmbedding export/editor split rules.
